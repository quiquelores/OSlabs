/*
 ...
 Course:    NYU Operating Systems Fall '13
...
PAGEFRAME_NUMBER tlb_lookup(VPAGE_NUMBER vpage, OPERATION op)
{
  ...
  int i;
  for(i = 0; i < num_tlb_entries; i++){
    if(((tlb[i].vbit_and_vpage & VPAGE_MASK) == (vpage &VPAGE_MASK))&&(tlb[i].vbit_and_vpage & VBIT_MASK)!=0){
      //set R bit of entry
      tlb[i].mr_pframe = (tlb[i].mr_pframe | RBIT_MASK);
      if(op == STORE)
	//set M bit of entry
                ...
                BOOL new_mbit, BOOL new_rbit)
{
    ...
    if(verbose)
        printf("Inserting page vpage: %x , pframe: %x into TLB\n",new_vpage, new_pframe);
    ...
    do{
        ...
        i = (i+1) & mod_tlb_entries_mask;  //otherwise increment i, using the mod
    ...
    if((tlb[i].vbit_and_vpage & VBIT_MASK) != 0){
        mmu_modify_rbit_bitmap((tlb[i].mr_pframe & PFRAME_MASK),(tlb[i].mr_pframe & RBIT_MASK)!=0);
        mmu_modify_mbit_bitmap((tlb[i].mr_pframe & PFRAME_MASK),(tlb[i].mr_pframe & MBIT_MASK)!=0);
    ...
    if(new_mbit)
        new_pframe= ( new_pframe | MBIT_MASK);
    
    if(new_rbit)
        new_pframe = (new_pframe | RBIT_MASK);
    
    tlb[i].mr_pframe = new_pframe;
    
    
    if(verbose)
        printf("instertion completed\n");
    
    // Finally, set clock_hand to point to the next entry after the
    // entry found above.
    
    clock_hand = (i + 1) & mod_tlb_entries_mask;
}
